#include "main.h"
/**
 * __print_octal - It prints unsigned integer
 ...
 */
int __print_octal(va_list list, char buff[], int sign, int extent,
int preceed, int sz)
{
...
int i = BUFF_SIZE - 2;
unsigned long int dig = va_arg(list, unsigned long int);
unsigned long int initial_dig = dig;
UNUSED(extent);
...
if (sign & HASH_SIGN && initial_dig != 0)
buff[i--] = '0';
i++;
 ...
 * Return: Always 0
 */
int __write_unsigned(int is_negative, int index, char buff[], int sign,
int extent, int preceed, int sz)
{
